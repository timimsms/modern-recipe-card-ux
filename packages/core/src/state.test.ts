...
import { createStore, progress, progressByStepCount, regionFill } from './state.js'
...
describe('the store', () => {
  it('notifies subscribers and stops when they unsubscribe', () => {
    ...
    store.toggleStep('melt')
    expect(calls).toBe(1)
    off()
    store.toggleStep('mix')
    expect(calls).toBe(1)
  })
  ...
  it('hands out a fresh snapshot on each change, so identity comparison works', () => {
    ...
    store.toggleIngredient('butter')
    expect(store.get()).not.toBe(before)
    expect(before.checkedIngredients.has('butter')).toBe(false)
    expect(store.get().checkedIngredients.has('butter')).toBe(true)
  })
  ...
  it('toggles rather than only setting', () => {
    ...
    store.toggleStep('melt')
    store.toggleStep('melt')
    expect(store.get().completedSteps.has('melt')).toBe(false)
  })
  ...
  it('undoes a check-off', () => {
    ...
    store.toggleIngredient('butter')
    store.toggleIngredient('sugar')
    expect(store.canUndo()).toBe(true)
    store.undo()
    expect(store.get().checkedIngredients.has('sugar')).toBe(false)
    expect(store.get().checkedIngredients.has('butter')).toBe(true)
  })
...
describe('timers', () => {
  ...
  it('counts down from the step duration', () => {
    ...
    store.startTimer(brownies.steps.bake!)
    expect(store.remaining('bake')).toBe(40 * 60_000)
    c.advance(10 * 60_000)
    expect(store.remaining('bake')).toBe(30 * 60_000)
  })
  ...
  it('survives a tab that slept, because nothing accumulates', () => {
    ...
    c.advance(39 * 60_000)
    expect(store.remaining('bake')).toBe(60_000)
    expect(store.isRinging('bake')).toBe(false)
    c.advance(60_000)
    expect(store.isRinging('bake')).toBe(true)
  })
  ...
  it('alerts at the low end of a range and keeps counting to the high end', () => {
    ...
    store.startTimer(ranged)
    c.advance(30 * 60_000)
    // "30–40 min" means start looking at 30, not that it is finished at 30.
    expect(store.isRinging('bake')).toBe(true)
    expect(store.get().timers.bake!.endsAfter).toBe(40 * 60_000)
  })
  ...
  it('runs several at once, which is the whole point of a parallel format', () => {
    ...
    const store = createStore({ now: c.now })
    store.startTimer(brownies.steps.bake!)
    store.startTimer(brownies.steps.melt!)
    c.advance(60_000)
    expect(store.remaining('bake')).toBe(39 * 60_000)
    expect(store.remaining('melt')).toBe(60_000)
    store.stopTimer('melt')
    expect(store.remaining('melt')).toBeNull()
  })

  it('ignores a step with no authored duration', () => {
    const store = createStore()
    store.startTimer({ id: 'x', inputs: [], text: 'x', effort: 'quick' })
    expect(store.remaining('x')).toBeNull()
  })
})
...
describe('progress', () => {
  it('weights by time, not by step count', () => {
    const store = createStore()
    for (const id of ['melt', 'mix', 'beat', 'fold']) store.toggleStep(id)
  ...
  it('is zero at the start and one at the end', () => {
    ...
    for (const id of Object.keys(brownies.steps)) store.toggleStep(id)
